...
async def analyser_et_stocker(payload: MesureRequest, db: Session = Depends(get_db)):
    """
    Recoit les 3 URLs Cloudinary depuis Laravel.
    Traite les images, stocke les mesures en DB et retourne le resultat.
    """

    # ── 1. Chercher la FicheMesure par external_id (UUID) ────────────
    fiche = db.query(FicheMesure).filter(
        FicheMesure.external_id == payload.fiche_id
    ).first()
    if not fiche:
        raise HTTPException(
    ...
    # ── 4. Supprimer les anciennes mesures de cette fiche ────────────
    db.query(Mesure).filter(Mesure.fiche_mesure_id == fiche.id).delete()

    # ── 5. Stocker les nouvelles mesures ─────────────────────────────
...
def get_mesures(fiche_id: str, db: Session = Depends(get_db)):
    """
    Retourne toutes les mesures d'une FicheMesure par external_id.
    """
    fiche = db.query(FicheMesure).filter(
        FicheMesure.external_id == fiche_id
    ).first()
    if not fiche:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
